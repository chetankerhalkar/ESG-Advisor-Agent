...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  if (!ENV.forgeApiKey) {
    throw new Error("OPENAI_API_KEY is not configured. Please add BUILT_IN_FORGE_API_KEY to your .env file");
  }
  ...
  const modelsToTry = [
    ENV.forgePrimaryModel?.trim(),
    ENV.forgeFallbackModel?.trim(),
  ].filter(
  ...
  const invokeModel = async (model: string): Promise<InvokeResult> => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `LLM invoke failed for model ${model}: ${response.status} ${response.statusText} - ${errorText}`
      );
    }

    const result = (await response.json()) as InvokeResult;
    console.log("[LLM] Success:", result.id);
    return result;
  };
  ...
  for (let i = 0; i < modelsToTry.length; i += 1) {
    ...
    try {
      ...
      if (nextModel) {
        console.warn(
          `[LLM] Model ${model} failed, retrying with fallback model ${nextModel}`
        );
      }
    }
  }
